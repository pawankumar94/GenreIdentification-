...
class FeatureGenerator:
    ...
    def removeHTMLTags(self, inp_string):
        ...
        clean_text = re.sub(pattern=reg_x, repl='', string=inp_string)
        return clean_text
    ...
    def addNoOfCharacters(self):
        ...
        for each in self.data['content'].iteritems():
            ...
            for token in nlp(each).ents:
                people += 1 if (token.label_ == "PERSON") else 0
            chars.append(people)
...
#init dataframe here
x = FeatureGenerator(pd.read_csv(r'./Res/full_data.csv', nrows=2))
